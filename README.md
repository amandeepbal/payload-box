...
With the new protected boxes (introduced in v2), you can even power your websites with https://payload-box.herokuapp.com/.
...
Base URL: `https://payload-box.herokuapp.com/`

### Create

You can create a record (or add a record) to a box by using HTTP post to `https://payload-box.herokuapp.com/${BOX_ID}`.

```sh
curl -X POST 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b' \
...
You can also pass in an optional collections parameter in the URL to group records `https://payload-box.herokuapp.com/${BOX_ID}/${COLLECTION}`.
...
```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b'
```
...
```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b/users'
```
...
```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b?sort=-age'
```

To read a specific record use `https://payload-box.herokuapp.com/${BOX_ID}/${RECORD_ID}`.

```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b/5d776a25fd6d3d6cb1d45c51'
```

To query records, you have to pass the key & value as shown below.

```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b?q=name:arya%20stark'
```
...
```sh
curl -X GET 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b?q=name:arya%20stark,age:>13'
```
...
```
https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b?q=name:arya%20stark,age:>13,isalive:true
```
...
```sh
curl -X PUT 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b/5d776b75fd6d3d6cb1d45c53' \
    -H 'content-type: application/json' \
...
- To delete a specific record use HTTP DELETE with https://payload-box.herokuapp.com/${BOX_ID}/${RECORD_ID}

```sh
curl -X DELETE 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b/5d776b75fd6d3d6cb1d45c53'
```

- To delete based on a filter use HTTP DELETE with https://payload-box.herokuapp.com/\${BOX_ID}?q={QUERY}

```sh
curl -X DELETE 'https://payload-box.herokuapp.com/demobox_6d9e326c183fde7b?q=name:arya%20stark,age:>13'
```
...
```
https://payload-box.herokuapp.com/_meta/demobox_6d9e326c183fde7b
```
...
```
git clone https://github.com/<your_name>/payload-box.git
```
...
to start the development server on port `3000`. Your payload-box instance will be running on `http://localhost:3000`. Alternatively you can run the application using docker with `docker-compose up`.
